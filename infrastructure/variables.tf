...
variable "station_name" {
  description = "Short lowercase name for this station (e.g. culfinbirds). Prefixes every AWS resource, names the ECR repo/ECS service, and selects stations/<station_name> as the baked-in profile."
  type        = string
...
variable "domain_name" {
  description = "Public apex domain served by CloudFront (e.g. culfinbirds.net)."
  type        = string
}

variable "github_repository" {
  description = "owner/name of the GitHub fork allowed to deploy via OIDC (e.g. benwalsh/culfinbirds)."
  type        = string
}
