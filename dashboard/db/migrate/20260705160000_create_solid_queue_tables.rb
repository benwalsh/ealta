# Solid Queue's tables, in the app's PRIMARY database (single-db, no separate `queue`
# database — keeps the cloud lean). Cloud-only in practice: only config/environments/
# cloud.rb sets the :solid_queue adapter and only Puma-in-cloud runs the worker, but
# the DDL is harmless on the Pi's SQLite (unused tables), matching the enrichment
# tables' convention so the whole thing stays dev-testable.
#
# Copied verbatim from solid_queue's install template (v1.4.0). Keep in sync on upgrade.
