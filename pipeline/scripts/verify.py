#!/usr/bin/env python3
"""ealta - adversarial species-ID + anatomy check on illustrations.
...
CSV_HEADER = (
    "slug,pose,target_sci,guessed_sci,guessed_com,matches,confidence,"
...
def main() -> int:
    ...
    ap.add_argument(
        "--out", type=Path, default=Path("verify-results.csv"), help="CSV output path (default: ./verify-results.csv)"
    )
    ...
    pngs = [args.dir / f"{s}.png" for s in args.slugs] if args.slugs else sorted(args.dir.glob("*.png"))
    ...
    for png in pngs:
        ...
        print(
            f"  {tag}{png.name}: reads as {v.get('guessed_species_com', '?')} "
