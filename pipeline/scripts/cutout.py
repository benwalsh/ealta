#!/usr/bin/env python3
"""ealta - cut the cream ground off the generated illustrations.
...
def main() -> int:
    ...
    ap.add_argument("slugs", nargs="*", help="Slugs to process (e.g. calypte-anna). Default: all.")
    ap.add_argument(
        "--dir", type=Path, default=None, help="Illustration directory (default: $STATION_PROFILE/illustrations/)"
    )
    ap.add_argument("--model", default="birefnet-general", help="rembg model name (default: birefnet-general)")
    ap.add_argument(
        "--margin",
        type=float,
        default=0.02,
    ...
    ap.add_argument("--force", action="store_true", help="Re-cut illustrations that already have transparency")
    args = ap.parse_args()
    args.dir = args.dir or art_dir()

    try:
        from PIL import Image
        from rembg import new_session, remove
    except ImportError:
        print("error: needs Pillow + rembg (pip install -r requirements.txt)", file=sys.stderr)
        return 2
