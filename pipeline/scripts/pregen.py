#!/usr/bin/env python3
"""ealta - generate kachō-e bird illustrations for a region.
...
the station profile's illustrations/ directory (see art.py).
...
<slug>-2.png (flight). Edit pipeline/scripts/prompt.template.md to change the
visual style - the prompt body is re-sent verbatim per request with
{sci_name}, {com_name}, and {pose} substituted.

Reference photos:
    Cached in pipeline/assets/references/. The auto-fetch hits the
...
Contrastive anti-reference:
    ...
    anti-reference photos live at pipeline/assets/references/_anti_*.jpg
...
# kept in a local directory (default pipeline/assets/references/styles/). Mapped by
...
ANTI_REFS = {
    "bluejay": {
        ...
        "do_not_copy": (
            "its facial mask, its white wingbars, its black necklace, its crest pattern, or its white-tipped tail"
        ),
    },
    "barnswallow": {
...
USER_AGENT = "ealta/1.0 (bird-station illustration pipeline)"


def slugify(sci: str) -> str:
    """Match the parent front-end (apt.js) slugify() exactly."""
...
def main() -> int:
    ...
    ap.add_argument(
        "--out", type=Path, default=None, help="Output directory (default: $STATION_PROFILE/illustrations/)"
    )
    ap.add_argument(
        "--refs",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "assets" / "references",
        help="Reference photo cache directory (default: pipeline/assets/references/)",
    )
    ap.add_argument(
        "--styles",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "assets" / "references" / "styles",
        help="Style reference directory (default: pipeline/assets/references/styles/)",
    )
    ap.add_argument(
        "--prompt",
        type=Path,
    ...
    ap.add_argument(
        "--no-refs", action="store_true", help="Skip the Wikipedia reference fetch (faster, lower-quality output)"
    )
    ap.add_argument(
        "--sleep",
        type=float,
